...
func (s *CheckInService) DoCheckIn(ctx context.Context, userID int64) (*CheckInStatus, error) {
	...
	if !config.Enabled {
		return nil, fmt.Errorf("签到功能暂未开放")
	}
...
func (s *CheckInService) DrawLottery(ctx context.Context, userID int64) (*LotteryResult, error) {
	...
	if !config.Enabled {
		return nil, fmt.Errorf("抽奖功能暂未开放")
	}
	...
	if drawCount >= config.DailyMaxDraws {
		return nil, fmt.Errorf("今日抽奖次数已用完")
	}
	...
	if lastRecord.TotalPoints < config.LotteryCost {
		return nil, fmt.Errorf("积分不足，需要%d积分，当前%d积分", config.LotteryCost, lastRecord.TotalPoints)
	}
...
// UpdateConfig 更新签到配置
func (s *CheckInService) UpdateConfig(ctx context.Context, updates map[string]interface{}) error {
	config, err := s.repo.GetCheckInConfig(ctx)
	if err != nil {
		return err
	}
...
// ListPrizes 获取所有奖品
func (s *CheckInService) ListPrizes(ctx context.Context) ([]*ent.LotteryPrize, error) {
...
// UpdatePrize 更新奖品
func (s *CheckInService) UpdatePrize(ctx context.Context, id int64, updates map[string]interface{}) error {
...
func (s *CheckInService) GetLotteryStats(ctx context.Context) (map[string]interface{}, error) {
	prizes, err := s.repo.ListPrizes(ctx)
	if err != nil {
		return nil, err
	}
